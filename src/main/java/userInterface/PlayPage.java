...
public class PlayPage extends StatePanel {
    ...
    public PlayPage(GameState gameState) {
            ...
            jButton.getCardButton().addMouseListener(new MouseAdapter() {
                public void mousePressed(MouseEvent event){
                    if(jButton.getCard() != null) {
                        player.writeLog("Click", "HandButton");
    ...
    private void setPlayedInitials(GameState gameState) {
        ...
        for(int i = 0; i <= 6; i++){
            ...
            jButton.getCardButton().addMouseListener(new MouseAdapter() {
                public void mousePressed(MouseEvent event){
                    if(jButton.getCard() != null) {
                        player.writeLog("Click", "PlayedButton");
                        jButton.setDoOpp(true);
                    }
                }
            });
    ...
    private void setDeckCards(GameState gameState) {
        ...
        jButton.setText(Integer.toString(gameState.getFirstDeck().size()) + " CARDS REMAINED");
        add(jButton);
        deckButton = jButton;
    }
    ...
    private void setEnd() {
        ...
        jButton.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent event){
                player.writeLog("Click", "EndButton");
                setEnd(true);
            }
        });
        add(jButton);
    ...
    private void checkShowHand() {
        for(StoreButton storeButton: handButtons){
            if(storeButton.isDoOpp()) {
                gameJFrame.changeState(this, new ShowCardPage(storeButton.getCard(), player,
    ...
    private void checkShowPlayed() {
        for(StoreButton storeButton: playedButtons){
            if(storeButton.isDoOpp()) {
                gameJFrame.changeState(this, new ShowCardPage(storeButton.getCard(), player, "", false,
                        gameState));
                storeButton.setDoOpp(false);
            }
        }
    ...
    public void getRenew() {
        ...
        deckButton.setText(Integer.toString(gameState.getFirstDeck().size()) + " CARDS REMAINED");
        showHand(gameState);
        showPlayed(gameState);
    }
